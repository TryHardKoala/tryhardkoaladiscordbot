  ...
  bot.on("message", async message => {
    ...
    fs.readFile("./data/prefixes.json", "utf8", async function (err, prefixes) {
            ...
            for (let i = 0; i < words.length; i++)
              for (let x = 0; x < filter.length; x++)
                if (filter[x].toLowerCase() == words[i].toLowerCase()) {
                  message.delete();
                  return message.channel.send(Embed({ title: 'Filter', description: lang.Filter_Message })).then(msg => { msg.delete(5000) });
                }
          }
          ...
          if (config.Anti_Advertisement_System == "true" && bypass && message.member.roles.sort((a, b) => b.calculatedPosition - a.calculatedPosition).first().calculatedPosition < bypass.calculatedPosition) {
            if (/(https?:\/\/)?([^\s]+)?[^\s]+\.[^\s]+/.exec(message.content)) {
              ...
              if (!whitelist.find(w => message.content.toLowerCase().includes(w.toLowerCase()))) {
                message.delete();
                return message.channel.send(Embed({ title: 'Anti-Advertisement', description: lang.Advertisement_Message })).then(msg => { msg.delete(5000) });
              }
            }
          }
        }
      ...
      if (bot.commands.size == 0) {
        if (cmd !== "activate") return;
        if (!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send(Embed({ preset: 'errorinfo', description: "You must have the ``ADMINISTRATOR`` permission to run this command!" }));
        if (args.length == 0) return message.channel.send(Embed({ preset: 'errorinfo', description: "Please provide an activation code", usage: 'activate <COREBOT KEY>' }));
        message.delete();
        ...
        request.post({
          ...
          .then(res => {
            if (typeof res == String && res.startsWith("Error")) {
              message.channel.send(Embed({ preset: 'errorinfo', description: "An error occured. Read console for more details." }));
      ...
      if (configMenu) {
        Object.keys(configMenu).forEach(async roleEmoji => {
          if (emojiKey == roleEmoji) {
            ...
            if (!role) return message.channel.send(Embed({ preset: 'errorinfo', description: `The ${configMenu[roleEmoji]} role does not exist.` })).then(msg => msg.delete(5000));
            if (member.roles.has(role.id)) {
              await member.removeRole(role);
              await message.channel.send(Embed({ description: member + ", You no longer have the ``" + configMenu[roleEmoji] + "`` role!" })).then(msg => { msg.delete(5000) });
            } else {
              await member.addRole(role);
              await message.channel.send(Embed({ description: member + ", You now have the ``" + configMenu[roleEmoji] + "`` role!" })).then(msg => { msg.delete(5000) });
            }
          }
        })
      }
